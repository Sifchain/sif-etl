...
def create_price_record_mutation(height, timestamp, rowan_cusdt, token_prices_dict, token_volumes_dict):
    ...
    sql_str = """
        ...
        from (select height, id, timestamp, json_object_keys(token_volumes_24hr) as tok from prices where height = '{0}') f 
        inner join (select p.* from prices p where height='{0}') p on f.height = p.height
        """.format(height)
